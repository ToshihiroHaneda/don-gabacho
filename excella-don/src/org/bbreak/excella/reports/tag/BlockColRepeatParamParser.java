 ...
 *
 * $Id: BlockColRepeatParamParser.java 95 2010-01-13 02:10:30Z tomo-shibata $
 * $Revision: 95 $
 *
...
//                    blockStartColIndex = tagCell.getColumnIndex();
                ...
                for ( int rowIdx = 0; rowIdx < blockCellsValue.length; rowIdx++) {
                    for ( int colIdx = 0; colIdx < blockCellsValue[rowIdx].length; colIdx++) {
                        ...
                        // セルの値の設定
                        PoiUtil.setCellValue( cell, blockCellsValue[rowIdx][colIdx]);

                        if ( blockCellsStyle[rowIdx][colIdx] != null) {
                            cell.setCellStyle( blockCellsStyle[rowIdx][colIdx]);
                        }
                        if ( log.isDebugEnabled()) {
                            log.debug( "row=" + (blockStartRowIndex + rowIdx) + " col" + (blockStartColIndex + colIdx) + ">>>>>>" + blockCellsValue[rowIdx][colIdx]);
                        }
                    }
                }
