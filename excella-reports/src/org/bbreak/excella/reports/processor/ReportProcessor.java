...
public class ReportProcessor {
     ...
     */
    public ByteArrayOutputStream process(ReportBook... reportBooks) throws Exception {
        
        ByteArrayOutputStream stream = new ByteArrayOutputStream();
        // 出力ブックデータ毎に処理する
        for ( ReportBook reportBook : reportBooks) {
            ByteArrayOutputStream bookStream = processBook( reportBook);
            stream.write(bookStream.toByteArray());
        }
        stream.flush();
        stream.close();
        return stream;
     ...
     * @throws ExportException 出力処理に失敗した場合
     */
    private ByteArrayOutputStream processBook(ReportBook reportBook) throws Exception {
        ...
        for ( ConvertConfiguration configuration : reportBook.getConfigurations()) {
            ...
            ReportBookExporter exporter = new ExcelOutputStreamExporter(outputStream);
            exporter.setConfiguration(configuration);
            controller.addBookExporter(exporter);
     ...
     * @throws IOException ファイルの読み込みに失敗した場合
     */
    private Workbook getTemplateWorkbook(InputStream stream) throws Exception {
